...
function issue_flannel(){
	cd $K8S_WORK_DIR
	mkdir -p flannel
	tar -xzvf $DIRNAME/flannel-v0.11.0-linux-amd64.tar.gz -C flannel
...
function write_etcd(){
	...
	etcdctl \
	  ...
	  mk ${FLANNEL_ETCD_PREFIX}/config '{"Network":"'${CLUSTER_CIDR}'", "SubnetLen": 24, "Backend": {"Type": "vxlan"}}'
          #${CLUSTER_CIDR}是子网掩码，一共32为掩码，/16表示前面的16位不动，后面的16位用来区分ip；SubnetLen：可以动的位数到哪里截止。21表示能动的只有17到21位，也就是可以产生2的4次方个ip
          #SubnetLen 我们独立环境填21，标准版环境填24
} 
